<?php
/**
 *    Page Hero
 * 
 *    
*/

if( !empty($fields['page_hero']['status']) ) :
	
	/**
	 * 
	 */

		$guides = [''];
		$return = [''];

	// /end
	
	/**
	 * 
	 */
		if ($fields['page_hero']['type'] == 'color__gradient') {


			/**
			 *  build the single color fill
			 */
				if( count($fields['page_hero']['color__gradient']) === 1 ) {
					$return['color__gradient-color'] = ' background: '.$fields['page_hero']['color__gradient'][0]['color'].';';
				}
			// 


			/**
			 *  build the linear_gradient if there are multiple color selections
			 */
				else if( !empty($fields['page_hero']['color__gradient']) ) {
					$guides['color__gradient-color'] = ', %1$s';
			
					$return['color__gradient-color'] = 'background-image: linear-gradient(to right';
					foreach( $fields['page_hero']['color__gradient'] as $i => $colorSelection ){
						$return['color__gradient-color'] .= sprintf(
							$guides['color__gradient-color']
							,$colorSelection['color']
						);
					}
					$return['color__gradient-color'] .= ');';
				}
			// 

			/**
			 * 	compile the entire section
			 */
				
				$guides['color__gradient'] .= '<section class="hero pagehero" style="%1$s"><div class="container%2$s%5$s">%3$s%4$s</div></section>';
				
				$return[0] .= sprintf(
					$guides['color__gradient']
					,$return['color__gradient-color']
					,' '.$fields['page_hero']['width']
					,!empty($fields['page_hero']['title']) ? '<h1>'.$fields['page_hero']['title'].'</h1>' : ''
					,!empty($fields['page_hero']['subtitle']) ? '<p>'.$fields['page_hero']['subtitle'].'</p>' : ''
					,!empty($fields['page_hero']['text_color']) ? ' light' : ' dark' 
				);

			// 
			
		}

	// 

	/**
	 * 
	 */

		if( $fields['page_hero']['type'] == 'image' ){



			$guides['image'] = '<section class="hero pagehero %1$s" style="%2$s"><div class="container%3$s%6$s">%4$s%5$s</div></section>';

			$return['image'] = '';
			
			$return[0] .= sprintf(
				$guides['image']
				,$fields['page_hero']['type']
				,'background-image: url('.$fields['page_hero']['image']['url'].');'
				,' '.$fields['page_hero']['width']
				,!empty($fields['page_hero']['title']) ? '<h1>'.$fields['page_hero']['title'].'</h1>' : ''
				,!empty($fields['page_hero']['subtitle']) ? '<p>'.$fields['page_hero']['subtitle'].'</p>' : ''
				,!empty($fields['page_hero']['text_color']) ? ' light' : ' dark' 
			);

			// 


		}
	 
	//  

	/**
	 * 
	 */
		if( $fields['page_hero']['type'] == 'slider' && !empty($fields['page_hero']['gallery']) ){


			$guides['slider'] = '<section class="hero pagehero %1$s">%6$s<div class="container%2$s%5$s">%3$s%4$s</div></section>';


			$return['slider'] = '<div class="pagehero-slider">';
			$return['slider-slide'] = '';
			$guide['slider-slide'] = '<div class="pagehero-slider-slide" style="background-image: url(%1$s)"></div>';

			foreach( $fields['page_hero']['gallery'] as $slide ){

				$return['slider'] .= sprintf(
					$guide['slider-slide']
					,$slide['url']
				);

			}
			$return['slider'] .= '</div>';

			
			
			
			$return[0] .= sprintf(
				$guides['slider']
				,$fields['page_hero']['type']
				,' '.$fields['page_hero']['width']
				,!empty($fields['page_hero']['title']) ? '<h1>'.$fields['page_hero']['title'].'</h1>' : ''
				,!empty($fields['page_hero']['subtitle']) ? '<p>'.$fields['page_hero']['subtitle'].'</p>' : ''
				,!empty($fields['page_hero']['text_color']) ? ' light' : ' dark' 
				,$return['slider']
			);
			

		}
	// 

	/**
	 * 
	 */

		if( $fields['page_hero']['type'] == 'video' && !empty($fields['page_hero']['video']) ){


			$guides['video'] = '<section class="hero pagehero %1$s">%6$s<div class="container%2$s%5$s">%3$s%4$s</div></section>';
			$return['video-video'] = '<video autoplay muted loop id="pagehero-video"><source src="'.$fields['page_hero']['video'].'" type="video/mp4"></video>';

			
			$return[0] .= sprintf(
				$guides['video']
				,$fields['page_hero']['type']
				,' '.$fields['page_hero']['width']
				,!empty($fields['page_hero']['title']) ? '<h1>'.$fields['page_hero']['title'].'</h1>' : ''
				,!empty($fields['page_hero']['subtitle']) ? '<p>'.$fields['page_hero']['subtitle'].'</p>' : ''
				,!empty($fields['page_hero']['text_color']) ? ' light' : ' dark'
				,$return['video-video']
			);

		}
	 
	 
	 

	/**
	 * 
	 */
	// /end 
	
	echo $return[0];



endif;

// 
?>